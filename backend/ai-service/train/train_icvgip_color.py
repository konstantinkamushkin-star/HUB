#!/usr/bin/env python3
"""
Обучение ICVGIPColorRestNet (Jain et al. ICVGIP'22 / arXiv:2211.14821) на парах input/target.
Экспорт: ../models/icvgip_color.onnx
...
from icvgip_color_net import ICVGIPColorRestNet
...
def main():
    ap = argparse.ArgumentParser(description="Train ICVGIP'22-style color restoration CNN")
    ...
    ap.add_argument("--export", type=Path, default=Path("../models/icvgip_color.onnx"))
    ap.add_argument("--checkpoint", type=Path, default=Path("../models/icvgip_color.pt"))
    args = ap.parse_args()
    ...
    print("Device:", device, "| ICVGIPColorRestNet | train:", n_tr, "val:", n_val)

    net = ICVGIPColorRestNet(base=48).to(device)
    ...
    try:
        ...
        print(f"ONNX (ICVGIP color CNN) -> {args.export.resolve()}")
