...
Ошибка **`Bind for 0.0.0.0:3000 failed: port is already allocated`** — на **хосте** порт уже занят **другим процессом** (старый контейнер, другой Nest и т.п.). Решение: в **`backend/.env`** задать **`API_PUBLISH_PORT=3001`** (или любой **свободный** порт) и в конфиге reverse proxy для API заменить `proxy_pass` на этот порт. Проверка: `ss -tlnp | grep 3000` и `docker ps`.
