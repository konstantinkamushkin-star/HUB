...
| Date | Change |
|------|--------|
| 2026-06-22 | **v1.2.0 Android parity batch**: feed video create/playback, dive editor video UVM, partner course editor route, booking confirmation screen, OAuth consent sheet, profile menu dedup, global search categories + debounce, statistics icons, inventory checkout signature pad, CreateTrip hotel/yacht URL fields, Play Store listing refresh. |
| Date | Change |
|------|--------|
| 2026-06-22 | **Android 1.2.0 parity release**: UVM Bech `ai2` cloud pipeline (`PhotoEnhancementProcessor`), logbook search/sort/swipe delete/share + detail route, feed hashtags + `divehub://hashtag`, unified profile edit, subscription trial UI, 7 languages in settings, version bump to 1.2.0. | production **FCM** requires replacing `google-services.json` with the real Firebase project; **Google OAuth** release builds need `google_oauth_web_client_id` (see strings/Gradle); **Stripe** env: `STRIPE_SECRET_KEY` / `STRIPE_PUBLISHABLE_KEY` in `backend/.env.local.example`. |
